"""Файлы обнаружения: карточка A2A, llms-full.txt, лента.
...
def test_card_has_no_unsubstituted_placeholders(client):
    for path in CARD_PATHS + ["/llms.txt", "/llms-full.txt"]:
...
def test_llms_full_is_complete_and_bounded(client):
    """Единственный ответ доски, которому позволено превысить потолок §6.
